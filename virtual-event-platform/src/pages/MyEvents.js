    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        const arrayData = Array.isArray(json)
          ? json
          : json.data?.bookings || json.data?.events || [];

        if (activeTab === 'registered') {
          // arrayData is bookings, extract eventId
          setAllEvents(arrayData.map(b => b.eventId).filter(e => e !== null));
        } else {
          // arrayData is all events, filter for created by current user
          setAllEvents(arrayData.filter(event => 
            event.createdBy === user?.id || 
            event.createdBy === user?._id || 
            (event.createdBy && (event.createdBy._id === user?._id || event.createdBy.id === user?.id)) ||
            (event.organizerEmail && user && event.organizerEmail.toLowerCase() === user.email.toLowerCase())
          ));
        }
      } catch (error) {
