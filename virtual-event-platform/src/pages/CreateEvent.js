...
function CreateEvent() {
  ...
  const [formData, setFormData] = useState({
    ...
    venueName: '',
    venueAddress: '',
    organizerName: user.fullName || user.name || '',
  ...
  const handleAiDraft = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/v1/intelligence/draft-event', {
        method: 'POST',
        headers: {
      ...
      if (response.ok && result.data && result.data.eventDraft) {
        const draft = result.data.eventDraft;
        
        // Coerce types to UI state expectation
        setFormData((prev) => ({
          ...prev,
          eventName: draft.eventName || prev.eventName,
          description: draft.description || prev.description,
          type: draft.type || prev.type,
          mode: draft.mode || prev.mode,
          category: draft.category || prev.category,
          language: draft.language || prev.language,
          // Handle HTML datetime-local format alignment (remove seconds/TZ)
          startDate: draft.startDate ? draft.startDate.substring(0, 16) : prev.startDate,
          endDate: draft.endDate ? draft.endDate.substring(0, 16) : prev.endDate,
          venueName: draft.venueName || prev.venueName,
          venueAddress: draft.venueAddress || prev.venueAddress,
          googleMapLink: draft.googleMapLink || prev.googleMapLink,
          meetingPlatform: draft.meetingPlatform || prev.meetingPlatform,
          ticketType: draft.ticketType || prev.ticketType,
          ticketPrice: draft.ticketPrice || prev.ticketPrice,
          attendeeLimit: draft.attendeeLimit || prev.attendeeLimit,
          registrationDeadline: draft.registrationDeadline ? draft.registrationDeadline.substring(0, 10) : prev.registrationDeadline
        }));

        setEventMode(draft.mode || '');
        setAiFeedback('✨ AI successfully drafted your event details! Review them below or edit them manually.');
        setCreationMode('manual'); // Transition user to manual review screen
        setActiveStep(1); // Start review at first step
      } else {
        alert('❌ AI Drafting Failed: ' + (result.message || 'Failed to analyze prompt'));
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        navigate(`/myevents`, { state: { activeTab: 'created' } });
      } else {
        alert('❌ Failed to create event: ' + (result.error || result.message || 'Unknown error'));
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="section-container pt-12 max-w-4xl">
        ...
        {/* Mode Switch Tabs */}
        <div className="flex bg-slate-950 p-1.5 rounded-2xl border border-slate-800/80 max-w-md mx-auto mb-10 shadow-inner">
          <button
            type="button"
          ...
          >
            <Sparkles className="h-4 w-4" /> AI Co-Creator
          </button>
          <button
            type="button"
          ...
          >
            <Info className="h-4 w-4" /> Manual Setup
          </button>
        </div>
        ...
        {creationMode === 'ai' ? (
          ...
          <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
            <div className="card bg-surface/50 border-slate-800/60 p-8 shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 h-40 w-40 bg-indigo-500/5 blur-[80px] rounded-full pointer-events-none" />
              <label className="block text-base font-bold text-text-primary mb-3">Describe your Event</label>
              <textarea
                value={aiPrompt}
              ...
              <div className="mt-6 flex flex-col md:flex-row items-center justify-between gap-4">
                <p className="text-xs text-text-secondary">AI extracts structure matching the Eventify Schema model.</p>
                <button
                  type="button"
                ...
                >
                  {isGenerating ? (
                    ...
                    <><Sparkles className="h-5 w-5 text-indigo-200" /> Draft with AI</>
                  )}
                </button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h4 className="text-xs font-bold uppercase tracking-widest text-slate-500 flex items-center gap-2">
                <Sparkles className="h-3.5 w-3.5 text-indigo-400" /> Try these prompt structures:
              </h4>
              <div className="grid grid-cols-1 gap-3">
                {samplePrompts.map((sample, idx) => (
                  <button
                    ...
                    className="p-4 rounded-xl bg-slate-950 border border-slate-900 text-left text-sm text-text-secondary hover:border-slate-800 hover:text-text-primary transition-all duration-200"
                  >
                    "{sample}"
                  </button>
                ))}
          ...
          /* Manual Step Form review */
          <div className="space-y-8 animate-in fade-in duration-500">
            {aiFeedback && (
              <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-2xl p-4 text-sm font-semibold flex items-center gap-3">
